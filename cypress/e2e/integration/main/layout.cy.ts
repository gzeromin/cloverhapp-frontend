describe('화면 사이즈 lg[960px] 이상', () => {
  ...
  it('화면 사이즈 lg[960px] 이상', () => {
    // Web 요소가 보여야 함
    cy.get('[data-cy="happCalendarSection"]').should('be.visible');
    cy.get('[data-cy="sideBarSection"]').should('be.visible');

    // Mobile 요소는 보이지 않아야 함
    cy.get('[data-cy="dayLogSection"]').should('not.be.visible');
  });
...
describe('화면 사이즈 md[768px] 이상', () => {
  ...
  it('화면 사이즈 md[768px] 이상', () => {
    ...
    cy.get('[data-cy="happCalendarSection"]').should('be.visible');
    cy.get('[data-cy="sideBarSection"]').should('not.be.visible');
...
describe('메인 페이지, 화면 사이즈 md[768px] 이하', () => {
  ...
  it('화면 사이즈 md[768px] 이하', () => {
    // Web 요소는 보이지 않아야 함
    cy.get('[data-cy="happCalendarSection"]').should('not.be.visible');
    cy.get('[data-cy="sideBarSection"]').should('not.be.visible');
    
    // Mobile 요소가 보여야 함
    cy.get('[data-cy="dayLogSection"]').should('be.visible');
  });
