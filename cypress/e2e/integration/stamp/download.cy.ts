...
describe('Download Page', () => {
  describe('소비자', () => {
    beforeEach(() => {
      ...
      cy.get('[id="downloadStamp-stampNameArea"]').as('stampNameArea');
      cy.get('[id="downloadStamp-registerArea"]').as('registerArea');
      cy.get('[id="downloadStamp-dropletArea"]').as('dropletArea');
      cy.get('[id="downloadStamp-descriptionArea"]').as('descriptionArea');
      cy.get('[id="downloadStamp-tagsArea"]').as('tagsArea');
      cy.get('[id="downloadStamp-cancelButton"]').as('cancelButton');
      cy.get('[id="downloadStamp-purchaseButton"]').as('purchaseButton');
    });

    it('초기 화면 표시 확인', () => {
      cy.get('[id="downloadStamp-notForSaleArea"]').should('not.exist');
      ...
      cy.get('[id="downloadStamp-deleteButton"]').should('not.exist');
      cy.get('[id="downloadStamp-editButton"]').should('not.exist');
    ...
    it('구입 버튼(성공)', () => {
      ...
      cy.get('[id="displayUserStampList"]').should('not.contain.text', 'Apple');
      ...
      cy.get('[id="displayUserStampList"]').should('contain.text', 'Apple');
    });
  ...
  describe('등록자', () => {
    beforeEach(() => {
      ...
      cy.get('[id="downloadStamp-dropletArea"]').as('dropletArea');
      cy.get('[id="downloadStamp-tagsArea"]').as('tagsArea');
      cy.get('[id="downloadStamp-deleteButton"]').as('deleteButton');
      cy.get('[id="downloadStamp-editButton"]').as('editButton');
      cy.get('[id="downloadStamp-cancelButton"]').as('cancelButton');
      cy.get('[id="downloadStamp-purchaseButton"]').as('purchaseButton');
    });

    it('초기 화면 표시 확인', () => {
      cy.get('[id="downloadStamp-notForSaleArea"]').should('exist');
      cy.get('@stampNameArea').should('contain.text', 'Apple');
      cy.get('@registerArea').should('contain.text', 'ddd');
      cy.get('@dropletArea').should('contain.text', '5');
      cy.get('[id="downloadStamp-descriptionArea"]').should('not.exist');
      cy.get('@tagsArea').should('contain.text', 'Speak');
